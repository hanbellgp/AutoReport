...
namespace Hanbell.AutoReport.EFGP
{
    public class WorkItemNeedSignConfig : NotificationConfig
    {
        ...
        public override void InitData()
        {
            ...
            sb.Append("WorkItem.createdTime,N'未签核' as status,DateDiff(DAY, WorkItem.createdTime, getdate()) AS delaydays FROM ProcessInstance,ParticipantActivityInstance,");
            sb.Append("WorkItem,WorkAssignment,Users WHERE ParticipantActivityInstance.contextOID = ProcessInstance.contextOID AND (ParticipantActivityInstance.currentState = 0 OR ParticipantActivityInstance.currentState = 1 OR  ParticipantActivityInstance.currentState = 6)  AND ");
            ...
            sb.Append("ProcessInstance.processDefinitionId NOT IN {0} ORDER BY Users.id,delaydays DESC ");
