...
{
    class YuqiweiqianheConfig_V : NotificationConfig
    {
        public YuqiweiqianheConfig_V()
        {
        }

        public YuqiweiqianheConfig_V(DBServerType dbType, string connName, string notification)
        {
            PrepareDBUtil(dbType, Base.GetDBConnectionString(connName));
            this.ds = new YuqiweiqianheDS();
        ...
        public override void InitData()
        {
            ...
            sb.Append("ProcessInstance.processDefinitionId IN {0} ORDER BY OrganizationUnit.id,Users.id,delaydays DESC ");

            Fill(String.Format(sb.ToString(), args["processDefinitionId"]), this.ds, "tblprocess1");
        }
    }
}
