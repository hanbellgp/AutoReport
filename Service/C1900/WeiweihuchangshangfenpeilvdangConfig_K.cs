...
namespace Hanbell.AutoReport.Config
{
    class WeiweihuchangshangfenpeilvdangConfig_K : NotificationConfig
    {
        ...
        public override void InitData()
        {
            ...
            sb.Append("or (a.itnbr not in (select purdis.itnbr from purdis  ) )) and d.pono not like 'AC%' and (a.itnbr not LIKE '%-9QC%' OR  len(a.itnbr) < 14 ) AND (substring(a.itnbr, 1, 14) not LIKE '%-C1%' OR substring(a.itnbr, 1, 14) not LIKE '%-C2%') AND len(substring(a.itnbr, 1, 14)) - len(STR_REPLACE(substring(a.itnbr, 1, 14), '-', NULL)) != 1");
