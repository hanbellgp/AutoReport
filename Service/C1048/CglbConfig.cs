...
{
    public class CglbConfig : NotificationConfig
    {
        public CglbConfig(DBServerType dbType, string connName)
        {
        ...
        public override void InitData()
        {
            ...
            isnull((select sum(y.acpamt) from apmpyh y ,invmas p,wlfl_db w where y.facno='C' and y.prono='1' and y.itnbr=p.itnbr  
        ...
        public override void ConfigData()
        {
            ...
            foreach (DataRow item in ds.Tables["tbcglb"].Rows)
            {
                ...
                if (decimal.Parse(item["ty"].ToString()) != 0)
                {
                    newrow[13] = String.Format("{0:N2}", 100 * decimal.Parse(item["ty"].ToString()) / puramtstotal);
                }
                else
                {
                    newrow[13] = "";
            ...
            foreach (DataRow item3 in ds.Tables["tblresult"].Rows)
            {
                    ...
                    if (decimal.Parse(item3["ly"].ToString()) != 0)
                    {
                        item3["lycz"] = decimal.Parse(String.Format("{0:N2}", 100 * (decimal.Parse(item3["ty"].ToString()) - decimal.Parse(item3["ly"].ToString())) / decimal.Parse(item3["ly"].ToString())));
                    }
