...
namespace Hanbell.AutoReport.VHB
{
    public class AccountReceivableDelay : NotificationContent
    {
        ...
        protected override void Init()
        {
            base.Init();
            string htmlFile = "";
            this.nc = new AccountReceivableDelayConfig(Core.DBServerType.SybaseASE, "VHBERP",this.ToString());
            nc.InitData();
            nc.ConfigData();
            ...
            if (nc.GetReportList().Count > 0)
            {
                ...
                foreach (ReportClass item in nc.GetReportList())
                {
                    ...
                    htmlFile = nc.ExportReportToHTML("AccountReceivableDelayReport_VHB_" + DateTime.Now.ToString("yyyy-MM-dd-H-mm-ss"), item);
