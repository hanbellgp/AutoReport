...
{
    public class AccountReceivableDelay_JN : AccountReceivableDelay
    {

        public AccountReceivableDelay_JN()
        {
        }
        ...
        protected override void Init()
        {
            SetMailHead();
            string htmlFile = "";
            this.nc = new AccountReceivableDelayConfig(Core.DBServerType.SybaseASE, "JNERP",this.ToString());
            nc.InitData();
            nc.ConfigData();
            ...
            if (nc.GetReportList().Count > 0)
            {
                ...
                foreach (ReportClass item in nc.GetReportList())
                {
                    ...
                    htmlFile = nc.ExportReportToHTML("AccountReceivableDelayReport_JN_" + DateTime.Now.ToString("yyyy-MM-dd-H-mm-ss"), item);
