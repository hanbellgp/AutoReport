...
{
    public class AccountReceivableDelay_GZ : AccountReceivableDelay
    {
        
        public AccountReceivableDelay_GZ()
        {
        }
        ...
        protected override void Init()
        {
            SetMailHead();
            string htmlFile = "";
            this.nc = new AccountReceivableDelayConfig(Core.DBServerType.SybaseASE, "GZERP",this.ToString());
            nc.InitData();
            nc.ConfigData();
            ...
            if (nc.GetReportList().Count > 0)
            {
                ...
                foreach (ReportClass item in nc.GetReportList())
                {
                    ...
                    htmlFile = nc.ExportReportToHTML("AccountReceivableDelayReport_GZ_" + DateTime.Now.ToString("yyyy-MM-dd-H-mm-ss"), item);
