...
{
    public class SpecOrderRDConfig : SpecOrderNotificationConfig
    {
        public SpecOrderRDConfig()
        {
        }

        public SpecOrderRDConfig(DBServerType dbType, string connName, string notification)
        {
        ...
        public override void InitData()
        {
            ...
            string sqlstr = "SELECT cdrspec.id,cdrspec.projname as project,cdrspec.itdsc as prodname,cdrspec.shipday1," +
                "cdrspecdetail.seq,cdrspecdetail.itdsc,cdrspecdetail.itnbr,cdrspecdetail.qty,cdrspecdetail.rdman as man," +
                "cdrspecdetail.rdday1 as day1,isnull(cdrspecdetail.remark,cdrspec.itnbr) as remark FROM cdrspec,cdrspecdetail   " +
                "WHERE ( cdrspec.facno = cdrspecdetail.facno ) and  ( cdrspec.id = cdrspecdetail.id ) and (cdrspec.status='Y') and (cdrspecdetail.state=0) " +
                "AND rdday2 is null AND datediff(day,getdate(),rdday1)<={0} AND cdrspec.kindcode='{1}'  ORDER BY cdrspec.facno ASC, cdrspec.id ASC    ";
            Fill(string.Format(sqlstr, args["rdday1"], args["kindcode"]), ds, "tblcdrspec");
