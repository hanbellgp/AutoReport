...
namespace Hanbell.AutoReport.Config
{
    public class ProductionOrderNeedCloseConfig : NotificationConfig
    {
        ...
        public override void InitData()
        {
            ...
            sb.Append("0 AS accqy1 ,0 AS okqy1 FROM    manmas,invmas b WHERE  manmas.itnbrf=b.itnbr and  manstatus IN ( 'A', 'B' )");
            ...
            sb.Append("vdrno ,keyindate ,mandate ,manqty ,finqty ,issqty ,");
            sb.Append("modqty = ISNULL(( SELECT SUM(modqty) FROM  manbor WHERE manbor.facno = manmas.facno AND manbor.prono = manmas.prono AND manbor.manno = manmas.manno ),0) ,");
            sb.Append("0 AS accqy1 ,0 AS okqy1 FROM  manmas,invmas b WHERE  manmas.itnbrf=b.itnbr and  manstatus < 'I' AND manstatus > 'D' ");
            ...
            sb.Append(" UNION ALL ");
            sb.Append("SELECT asspurhad.manno ,purhad.pono ,itnbrf ,b.itdsc,");
            sb.Append(" purhad.vdrno ,  keyindate , mandate , manqty , finqty ,issqty ,");
            sb.Append("modqty = ISNULL(( SELECT SUM(modqty) FROM  manbor WHERE manbor.facno = manmas.facno AND manbor.prono = manmas.prono AND manbor.manno = manmas.manno ),0) ,");
            sb.Append("purdta.accqy1 ,purdta.okqy1 FROM    purhad , purdta ,asspurhad ,manmas,invmas b ");
            sb.Append(" WHERE  manmas.itnbrf=b.itnbr and  ( purdta.facno = purhad.facno ) AND ( purdta.pono = purhad.pono ) AND ( purhad.prono = purdta.prono ) AND manmas.manno = asspurhad.manno ");
