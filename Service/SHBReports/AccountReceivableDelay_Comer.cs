...
{
    public class AccountReceivableDelay_Comer:AccountReceivableDelay
    {

        public AccountReceivableDelay_Comer()
        {
        }
        ...
        protected override void Init()
        {
            SetMailHead();
            string htmlFile = "";
            this.nc = new AccountReceivableDelayConfig(Core.DBServerType.SybaseASE, "ComerERP",this.ToString());
            nc.InitData();
            nc.ConfigData();
            ...
            if (nc.GetReportList().Count > 0)
            {
                ...
                foreach (ReportClass item in nc.GetReportList())
                {
                    ...
                    htmlFile = nc.ExportReportToHTML("AccountReceivableDelayReport_Comer_" + DateTime.Now.ToString("yyyy-MM-dd-H-mm-ss"), item);
